...
1. Run `.blitz/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. All paths must be absolute. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").
...
3. Load and analyze the implementation context:
   ...
   - **IF EXISTS**: Read /memory/playbook.md for governance constraints
...
6. Executor routing (local model delegation):
   - Check whether tasks.md contains `[L]` markers AND the `blitz` CLI is on PATH
     (`command -v blitz`). If either is missing, skip this step and execute every task yourself.
   - `[L]` marks tasks the user's configured local model (see `local_model` in
     `.blitz/config.json`) can execute. Delegate those instead of doing them yourself —
   ...
   - Work through the task list in ID order with this loop:
     ...
     2. If it is marked `[L]`: run `blitz model execute <path-to-tasks.md> --local-only --collect --start-at <TaskID>`
        ...
        model failed, do NOT mark its sft_queue entry resolved yourself; your solution
        is frontier output and must never be recorded as a local-model attempt.
