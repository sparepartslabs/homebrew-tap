...
**Playbook Authority**: The project playbook (`/memory/playbook.md`) is **non-negotiable** within this analysis scope. Playbook conflicts are automatically CRITICAL and require adjustment of the spec, plan, or tasks—not dilution, reinterpretation, or silent ignoring of the principle. If a principle itself needs to change, that must occur in a separate, explicit playbook update outside `/analyze`.

## Execution Steps

### 1. Initialize Analysis Context

Run `.blitz/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` once from repo root and parse JSON for FEATURE_DIR and AVAILABLE_DOCS. Derive absolute paths:
