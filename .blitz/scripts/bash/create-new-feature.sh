...
# Resolve repository root using common.sh functions which prioritize .blitz
...
if [ "$DRY_RUN" != true ]; then
    ...
    # Persist to .blitz/feature.json so downstream commands can find the feature
    _persist_feature_json "$REPO_ROOT" "$FEATURE_DIR"

    # Inform the user how to set feature state in their own shell
    printf '# To persist: export PLAYBOOKS_FEATURE=%q\n' "$BRANCH_NAME" >&2
    printf '#              export PLAYBOOKS_FEATURE_DIRECTORY=%q\n' "$FEATURE_DIR" >&2
fi
...
if $JSON_MODE; then
    ...
    if [ "$DRY_RUN" != true ]; then
        printf '# To persist in your shell: export PLAYBOOKS_FEATURE=%q\n' "$BRANCH_NAME"
        printf '#                           export PLAYBOOKS_FEATURE_DIRECTORY=%q\n' "$FEATURE_DIR"
    fi
fi
