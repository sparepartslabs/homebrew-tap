...
# Find repository root by searching upward for .blitz directory
# This is the primary marker for playbooks projects
find_playbooks_root() {
    local dir="${1:-$(pwd)}"
...
# Resolve an explicit PLAYBOOKS_INIT_DIR project override (the directory that
# *contains* .blitz/), for non-interactive / CI use — e.g. running a Playbooks
# command against a member project from a monorepo root without cd.
#
# Precondition: PLAYBOOKS_INIT_DIR is non-empty. Echoes the validated absolute
# project root, or prints an error and returns 1. Strict by design: the path
# must exist and contain .blitz/, with no silent fallback to cwd or the
# script-location default (which would silently write to the wrong project).
#
# This is the single resolver: bundled extensions inherit it by sourcing core
# (e.g. the git extension's create-new-feature-branch) rather than duplicating it.
resolve_playbooks_init_dir() {
    local init_root
    ...
    if ! init_root="$(CDPATH="" cd -- "$PLAYBOOKS_INIT_DIR" 2>/dev/null && pwd)"; then
        echo "ERROR: PLAYBOOKS_INIT_DIR does not point to an existing directory: $PLAYBOOKS_INIT_DIR" >&2
        return 1
    fi
    if [[ ! -d "$init_root/.blitz" ]]; then
        echo "ERROR: PLAYBOOKS_INIT_DIR is not a Playbooks project (no .blitz/ directory): $init_root" >&2
        return 1
    fi
    printf '%s\n' "$init_root"
}

# Get repository root, prioritizing .blitz directory
# This prevents using a parent repository when playbooks is initialized in a subdirectory
get_repo_root() {
    # Explicit project override wins (see resolve_playbooks_init_dir).
    if [[ -n "${PLAYBOOKS_INIT_DIR:-}" ]]; then
        resolve_playbooks_init_dir
        return
    fi

    # First, look for .blitz directory (playbooks's own marker)
    local playbooks_root
    if playbooks_root=$(find_playbooks_root); then
        echo "$playbooks_root"
        return
    fi
...
# Feature state is set by PLAYBOOKS_FEATURE (from create-new-feature or
# the git extension) or implicitly via .blitz/feature.json.
get_current_branch() {
    if [[ -n "${PLAYBOOKS_FEATURE:-}" ]]; then
        echo "$PLAYBOOKS_FEATURE"
        return
    fi
...
# Safely read .blitz/feature.json's "feature_directory" value.
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.blitz/feature.json"
...
# Persist a feature_directory value to .blitz/feature.json.
...
get_feature_paths() {
    ...
    # --no-persist so pure path resolution never writes .blitz/feature.json,
    ...
    # Split decl/assignment so a PLAYBOOKS_INIT_DIR validation failure in
    ...
    #   1. PLAYBOOKS_FEATURE_DIRECTORY env var (explicit override)
    #   2. .blitz/feature.json "feature_directory" key (persisted by specify command)
    #   3. Error — no feature context available
    local feature_dir
    if [[ -n "${PLAYBOOKS_FEATURE_DIRECTORY:-}" ]]; then
        feature_dir="$PLAYBOOKS_FEATURE_DIRECTORY"
        ...
        if [[ "$no_persist" != true ]]; then
            _persist_feature_json "$repo_root" "$PLAYBOOKS_FEATURE_DIRECTORY"
        fi
    elif [[ -f "$repo_root/.blitz/feature.json" ]]; then
        local _fd
        ...
        if [[ -n "$_fd" ]]; then
            ...
            echo "ERROR: Feature directory not found. Set PLAYBOOKS_FEATURE_DIRECTORY or ensure .blitz/feature.json contains feature_directory." >&2
            return 1
        fi
    else
        echo "ERROR: Feature directory not found. Set PLAYBOOKS_FEATURE_DIRECTORY or run the specify command to create .blitz/feature.json." >&2
        return 1
    fi

    # When no branch context exists (no PLAYBOOKS_FEATURE, feature resolved via
    # PLAYBOOKS_FEATURE_DIRECTORY or feature.json), fall back to the feature
...
get_invoke_separator() {
    ...
    if [[ "${_PLAYBOOKS_INVOKE_SEPARATOR_CACHE_REPO_ROOT:-}" == "$repo_root" && -n "${_PLAYBOOKS_INVOKE_SEPARATOR_CACHE_VALUE:-}" ]]; then
        printf '%s\n' "$_PLAYBOOKS_INVOKE_SEPARATOR_CACHE_VALUE"
        return 0
    fi

    local integration_json="$repo_root/.blitz/integration.json"
    local separator="."
    local parsed=0
...
PY
    ...
    _PLAYBOOKS_INVOKE_SEPARATOR_CACHE_REPO_ROOT="$repo_root"
    _PLAYBOOKS_INVOKE_SEPARATOR_CACHE_VALUE="$separator"
    printf '%s\n' "$separator"
}

format_playbook_command() {
    # Playbooks commands install without a namespace (e.g. /specify, /plan),
    # so a command name maps directly to its slash-command.
...
#   1. .blitz/templates/overrides/
#   2. .blitz/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .blitz/extensions/<ext-id>/templates/
#   4. .blitz/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.blitz/templates"
    ...
    local presets_dir="$repo_root/.blitz/presets"
    if [ -d "$presets_dir" ]; then
        ...
        if [ -f "$registry_file" ] && command -v python3 >/dev/null 2>&1; then
            ...
            if sorted_presets=$(PLAYBOOKS_REGISTRY="$registry_file" python3 -c "
import json, sys, os
try:
    with open(os.environ['PLAYBOOKS_REGISTRY']) as f:
        data = json.load(f)
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.blitz/extensions"
    if [ -d "$ext_dir" ]; then
                ...
                if [ -n "$sorted_presets" ]; then
                    ...
                    while IFS= read -r preset_id; do
                        ...
                        if [ -f "$manifest" ] && command -v python3 >/dev/null 2>&1; then
                            ...
                            local py_stderr
                            py_stderr=$(mktemp)
                            result=$(PLAYBOOKS_MANIFEST="$manifest" PLAYBOOKS_TMPL="$template_name" python3 -c "
import sys, os
try:
    import yaml
except ImportError:
    ...
    sys.exit(0)
try:
    with open(os.environ['PLAYBOOKS_MANIFEST']) as f:
        data = yaml.safe_load(f)
    for t in data.get('provides', {}).get('templates', []):
        if t.get('name') == os.environ['PLAYBOOKS_TMPL'] and t.get('type', 'template') == 'template':
